...
grafico2<-ggplot(data=ds, aes(x=Temp9am, y=Pressure9am, shape=RainToday)) +
  geom_point()

grafico3<-ggplot(data=ds, aes(x=Temp9am, y=Pressure9am, color=RainToday)) +
  geom_point()


grafico_final<-grafico1 + labs(title="Relación entre Temperatura y Presión a las 9 am", subtitle="Coloreado según si llovió (RainToday)", x="Temperatura a las 9 am (°C)", y="Presión Atmosférica a las 9 am (hPa)", color="Lluvia Hoy") 
...
iris.grafico3_filas<-iris.grafico2+facet_grid(Measure ~ .) 
iris.grafico3_filas

iris.grafico3_columnas<-iris.grafico2+facet_grid(. ~ Measure) 
iris.grafico3_columnas

iris.grafico4_jitter<-ggplot(data=iris.tidy,aes(x=Species, y=Value, color=Part)) +
  geom_jitter(width=0.2, alpha=0.5)+ facet_grid(. ~ Measure) 
iris.grafico4_jitter

ggsave('figs/Ejercicio3.pdf') 
...
*Si quisiera cambiar las formas de los circulos a cuadrados de las lineas como sería
...
ggplot(data = ice.river.RData)
ggplot(data=ice.river,df, aes(x=time()))

```
#si no reorganizacion de los datos flujo y y color rio y x time, 
